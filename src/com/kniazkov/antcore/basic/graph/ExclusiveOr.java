...
public class ExclusiveOr extends BinaryOperation {
    public ExclusiveOr(Expression left, Expression right) {
        super(left, right);
    }
    ...
    public void genLoad(CompilationUnit unit) throws SyntaxError {
        ...
        if (leftType instanceof ShortType && rightType instanceof ShortType) {
            unit.addInstruction(new Xor(TypeSelector.SHORT,2, 2, 2));
            return;
        }

        if (leftType instanceof IntegerType && rightType instanceof IntegerType) {
            unit.addInstruction(new Xor(TypeSelector.INTEGER,4, 4, 4));
            return;
        }
